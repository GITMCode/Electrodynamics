...
!-----------------------------------------------------------------------
    ...
    subroutine read_all_files(inDir)
      use ModCharSize
      implicit none
...
!-----------------------------------------------------------------------
end module ModW05_read_data

!-----------------------------------------------------------------------
module w05sc
  use ModW05_read_data,only: csize
  implicit none
!
...
!-----------------------------------------------------------------------
  subroutine setboundary(angle,bt,tilt,swvel,swden)
    use ModW05_read_data,only: read_bndy,ex_bndy,na,bndya ! na=6
    implicit none
!
! Args:
!
...
!-----------------------------------------------------------------------
  subroutine epotval(lat,mlt,fill,epot)
    use ModW05_read_data,only: maxm_pot,ms,ab
    implicit none
!
! Args:
...
!-----------------------------------------------------------------------
  subroutine mpfac(lat,mlt,fill,fac)
    use ModW05_read_data,only: ls,ms,ab
    implicit none
!
! Args:
    real,intent(in) :: lat,mlt,fill
...
!-----------------------------------------------------------------------
  real function nkmlookup(k,m,th0)
    use ModW05_read_data,only: d1_scha,d2_scha,d3_scha ! d1=19, d2=7, d3=68
    use ModW05_read_data,only: allnkm,th0s             ! allnkm(d1,d2,d3),th0s(d3)
    use ModW05_read_data,only: maxk_scha,maxm_scha
    implicit none
!
! Args:
